...
class Meetings(commands.Cog):
    ...
    async def _playgame(self, ctx, time, theme):
        try:
            ...
            logging.warning(f"{ctx.author} entered a wrong time for the gamecommand!")
            return

        group = {f"{ctx.channel.name}": {"id": ctx.channel.id, "owner": ctx.author.mention, "theme": theme, "time": GameDateTimeTimestamp, "members": [f"{ctx.author.mention}"]}}

        try:
            if ctx.channel.name in self.bot.Settings["Settings"]["Groups"]:
                ...
                logging.info(f"Meeting started in {ctx.channel.name} with theme: [{theme}] at {time}.")
        except json.decoder.JSONDecodeError:
            self.bot.Settings = _read_json("Settings.json")
            self.bot.Settings["Settings"]["Groups"].update(group)
            _write_json("Settings.json", self.bot.Settings)
            await ctx.respond(f"Der Erinnerer wurde erstellt mit dem Thema {theme} um {time}!")
            logging.warning(f"The settingsfile is corrupted, overwrote the file and started meeting in {ctx.channel.name}!")
    ...
    async def _showgame(self, ctx):
        ...
        if ctx.channel.name in GameSettings["Settings"]["Groups"]:
            CleanUsernames = []
            ...
            ReminderTime = datetime.datetime.fromtimestamp(GameSettings["Settings"]["Groups"][f"{CurrentChannel}"]["time"], tz=datetime.datetime.utcnow().astimezone().tzinfo)
    ...
    async def _joingame(self, ctx):
        CurrentChannel = ctx.channel.name
        if ctx.channel.name in self.bot.Settings["Settings"]["Groups"] and ctx.author.mention in self.bot.Settings["Settings"]["Groups"][f"{CurrentChannel}"]["members"]:
            await ctx.respond(f"{ctx.author.name}, du bist bereits als Teilnehmer im geplanten Erinnerer.")
        elif ctx.channel.name in self.bot.Settings["Settings"]["Groups"] and f"{ctx.author.mention}" not in self.bot.Settings["Settings"]["Groups"][f"{CurrentChannel}"]["members"]:
            self.bot.Settings["Settings"]["Groups"][f"{CurrentChannel}"]["members"].append(f"{ctx.author.mention}")
            await ctx.respond(f"{ctx.author.mention}, du wurdest dem Erinnerer hinzugefügt.")
            logging.info(f"{ctx.author} joined the meeting in {ctx.channel.name}.")
            _write_json("Settings.json", self.bot.Settings)
        else:
    ...
    async def _movegame(self, ctx, time):
        CurrentChannel = ctx.channel.name
        if ctx.channel.name in self.bot.Settings["Settings"]["Groups"] and ctx.author.mention == self.bot.Settings["Settings"]["Groups"][f"{CurrentChannel}"]["owner"]:
            try:
                CurrentDate = datetime.datetime.now()
                GameTime = datetime.datetime.strptime(time, "%H:%M").time()
                GameDateTime = datetime.datetime.combine(CurrentDate, GameTime)
                ...
                await ctx.respond("Na na, das ist keine Uhrzeit!")
                logging.warning(f"{ctx.author} entered a wrong time for the updategamecommand!")
                return
            ...
            logging.info(f"{ctx.author} moved the meeting in {CurrentChannel} for {ReminderTheme} to {time}.")
        elif ctx.channel.name in self.bot.Settings["Settings"]["Groups"] and ctx.author.mention != self.bot.Settings["Settings"]["Groups"][f"{CurrentChannel}"]["owner"]:
            await ctx.respond("Na na, du bist nicht der Besitzer dieser Verabredung! Frag bitte den Besitzer, ob er es verschiebt!")
            logging.warning(f"{ctx.author} tried to move the meeting for {ReminderTheme} in {CurrentChannel} but is not the owner!")
        elif ctx.channel.name not in self.bot.Settings["Settings"]["Groups"]:
    ...
    async def _gameremover(self, ctx):
        ...
        if ctx.channel.name in self.bot.Settings["Settings"]["Groups"] and ctx.author.mention == self.bot.Settings["Settings"]["Groups"][f"{CurrentChannel}"]["owner"]:
            ...
            logging.warning(f"{ctx.author} tried to delete a meeting in {CurrentChannel} for {ReminderTheme} but is not the owner!")
        elif ctx.channel.name not in self.bot.Settings["Settings"]["Groups"]:
    ...
    async def _leavegame(self, ctx):
        ...
        ReminderTime = datetime.datetime.fromtimestamp(StartedGroups["Settings"]["Groups"][f"{CurrentChannel}"]["time"], tz=datetime.datetime.utcnow().astimezone().tzinfo)
        if ctx.channel.name in StartedGroups["Settings"]["Groups"] and ctx.author.mention == StartedGroups["Settings"]["Groups"][f"{CurrentChannel}"]["owner"]:
            ...
            logging.info(f"{ctx.author} left the meeting for {ReminderTheme} in {CurrentChannel} and was the owner. Meeting deleted.")
        elif ctx.channel.name in StartedGroups["Settings"]["Groups"] and ctx.author.mention in StartedGroups["Settings"]["Groups"][f"{CurrentChannel}"]["members"]:
            StartedGroups["Settings"]["Groups"][f"{CurrentChannel}"]["members"].remove(ctx.author.mention)
            await ctx.respond(f"{ctx.author.mention}, du wurdest aus dem Erinnerer für {ReminderTheme} um {ReminderTime:%H:%M} entfernt.")
            logging.info(f"{ctx.author} removed from meeting for {ReminderTheme} in {CurrentChannel}.")
            _write_json("Settings.json", StartedGroups)
        elif ctx.channel.name in StartedGroups["Settings"]["Groups"] and ctx.author.mention not in StartedGroups["Settings"]["Groups"][f"{CurrentChannel}"]["members"]:
            ...
            logging.info(f"{ctx.author} wanted to leave the meeting for {ReminderTheme} in {CurrentChannel}, but was not a member.")
        else:
            await ctx.respond(f"{ctx.author.mention}, hier gibt es keinen Erinnerer, den du verlassen könntest.")
            logging.info(f"{ctx.author} wanted to leave a meeting for {ReminderTheme} in {CurrentChannel}, but there was none.")
    ...
    async def _playgame_error(self, ctx, error):
        if isinstance(error, commands.MissingRequiredArgument):
            ...
            logging.warning(f"{ctx.author} wanted to meet outside of an entertainment channel!")
    ...
    async def _leavegame_error(self, ctx, error):
        if isinstance(error, discord.errors.CheckFailure):
            ...
            logging.warning(f"{ctx.author} wanted to leave a meeting outside of an entertainment channel!")
    ...
    async def _showgame_error(self, ctx, error):
        if isinstance(error, commands.CommandOnCooldown):
            ...
            logging.warning(f"{ctx.author} wanted to spam the members of a game in {ctx.channel.name}!")
